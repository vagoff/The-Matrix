...
L3:
        uw_error(ctx, FATAL, "/mnt/sources/ur/funpat.ur:3:10-8:0: pattern match failure");
L0: result;
    })),
...
static void uw_handle(uw_context ctx, char *request)
{
    if (!strcmp(request, "/app.js"))
    {
        ...
        if (ims && !strcmp(ims, "Sun, 02 Jan 2011 23:49:43"))
        {
        ...
        uw_write_header(ctx, "Last-modified: Sun, 02 Jan 2011 23:49:43\r\n");
        uw_write(ctx, jslib);
        return;
    }
    if (!strncmp(request, "/Funpat/main", 12) && (request[12] == 0 || request[12] == '/'))
    {
        request += 12;
        if (*request == '/') ++request;
